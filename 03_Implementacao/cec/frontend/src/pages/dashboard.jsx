...
import {
  ...
  criarPasta, atualizarPasta, eliminarPasta, getModeloStatus, retreinarModelo, alterarTipoModelo } from "../js/api/client.jsx";
...
export default function Dashboard() {
  ...
  const abrirCriarProjeto = () => {
    setInputNome(""); setInputDesc(""); setModalProjeto("criar");
  };

  const abrirEditarProjeto = (proj) => {
    setInputNome(proj.nome); setInputDesc(proj.descricao || ""); setModalProjeto(proj);
  };
  ...
  const confirmarProjeto = async () => {
    ...
    if (modalProjeto === "criar") {
      ...
      setProjetosCompletos((prev) => { const c = { ...prev }; delete c[modalProjeto.id]; return c; });
    }
  ...
  const confirmarEliminarProjeto = async (id) => {
    ...
    setProjetosCompletos((prev) => { const c = { ...prev }; delete c[id]; return c; });
    carregar();
  };

  const abrirCriarPasta = (projetoId) => {
    setInputNome(""); setModalPasta({ tipo: "criar", projetoId });
  };
            ...
            <div key={proj.id} className="projeto-card">
              <div className="projeto-card-header" onClick={() => toggleProjeto(proj.id)}>
                ...
                <div className="projeto-meta">
                  ...
                  <button className="btn-icon" onClick={(e) => { e.stopPropagation(); abrirEditarProjeto(proj); }} title="Editar">✏️</button>
                  <button className="btn-icon btn-danger" onClick={(e) => { e.stopPropagation(); confirmarEliminarProjeto(proj.id); }} title="Eliminar">🗑</button>
              ...
              {projetoExpandido === proj.id && (
                <div className="projeto-pastas">
                  {pastas.length === 0 && <p className="pastas-empty">Sem pastas. Cria a primeira.</p>}
                  {pastas.map((pasta) => (
                    <div key={pasta.id} className="pasta-row">
                      <button className="pasta-btn" onClick={() => abrirPasta(proj.id, pasta.id)}>
                        ...
                        <span className="pasta-count">{pasta.total_noticias} notícia{pasta.total_noticias !== 1 ? "s" : ""}</span>
                      </button>
                      <button className="btn-icon" onClick={() => abrirEditarPasta(proj.id, pasta)} title="Editar">✏️</button>
                      <button className="btn-icon btn-danger" onClick={() => confirmarEliminarPasta(proj.id, pasta.id)} title="Eliminar">🗑</button>
                    </div>
                  ))}
                  <button className="btn-nova-pasta" onClick={() => abrirCriarPasta(proj.id)}>+ Nova pasta</button>
                </div>
              )}
            </div>
          );
      ...
      {modalProjeto && (
        <div className="modal-overlay" onClick={() => setModalProjeto(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <label>Nome</label>
            <input autoFocus value={inputNome} onChange={(e) => setInputNome(e.target.value)} onKeyDown={(e) => e.key === "Enter" && confirmarProjeto()} placeholder="Nome do projeto" />
            <label>Descrição (opcional)</label>
            <input value={inputDesc} onChange={(e) => setInputDesc(e.target.value)} placeholder="Descrição" />
            <div className="modal-actions">
              <button className="btn-cancelar" onClick={() => setModalProjeto(null)}>Cancelar</button>
              <button className="btn-confirmar" onClick={confirmarProjeto}>{modalProjeto === "criar" ? "Criar" : "Guardar"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalPasta && (
        <div className="modal-overlay" onClick={() => setModalPasta(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <label>Nome</label>
            <input autoFocus value={inputNome} onChange={(e) => setInputNome(e.target.value)} onKeyDown={(e) => e.key === "Enter" && confirmarPasta()} placeholder="Nome da pasta" />
            <div className="modal-actions">
              <button className="btn-cancelar" onClick={() => setModalPasta(null)}>Cancelar</button>
              <button className="btn-confirmar" onClick={confirmarPasta}>{modalPasta.tipo === "criar" ? "Criar" : "Guardar"}</button>
            </div>
          </div>
        </div>
      )}
